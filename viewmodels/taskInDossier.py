...
def definetasktableobject(tasktoupdate):
    if tasktoupdate is not None:
        ...
        return [taskID, taskname, taskdatelim,  taskstatus, taskIsdone]
    else:
        return None
...
class allTodoTableViewInDossier(QAbstractTableModel):
    def __init__(self, allTodoData, parent=None):
        ...
        self.header = ['ID', 'A faire', 'Date butoire', 'Statut', '']
    ...
    def data(self, index, role):
        ...
        if role == Qt.BackgroundRole:
            if self.mydata.item(index.row(), 3).data(QtCore.Qt.DisplayRole)== "Réalisée":
